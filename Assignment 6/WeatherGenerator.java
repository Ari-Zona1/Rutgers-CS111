...
public class WeatherGenerator {
    ...
    public static void populateLocationProbabilities( double[] drywetProbability, double[] wetwetProbability, 
        ...
        for(int i = 0; i < drywet.length; i++){
            ...
            //Finds where the longitude and latitude are equal 
            if(drywet[i][0] == longitude && drywet[i][1] == latitude){
                
                //loops through the arrays and poplates the probabilities
                for(int j = 0; j < 12; j++){
                    
                    drywetProbability[j] = drywet[i][j+2];
                    //System.out.println(drywetProbability[j]);
                    wetwetProbability[j] = wetwet[i][j+2];
                    //System.out.println(wetwetProbability[j]);
                }
            }
        }
    }
    ...
    public static int[] forecastGenerator( double drywetProbability, double wetwetProbability, int numberOfDays) {
        ...
        // Create int[]array and intialize it to the size of numberOfDays
        int[] forecastArr = new int[numberOfDays]; 

        //Create double variables 
        double probability = 0.5;
        double realNum = StdRandom.uniform(); //to generate a real number uniformly in [0,1)

        //Calculates first day of the month
        //Real num is greater than our probability rate of 50% then DRY
        //else WET
        if(probability < realNum){
            forecastArr[0] = DRY; 
        }else{
            forecastArr[0] = WET;
        }

        //Checking the probability of the pervious day and taking that probability to see if it will be a dry or wet day
        for (int i = 1; i < forecastArr.length; i++) {
            
            //get new real number
            realNum = StdRandom.uniform();
            
            switch (forecastArr[i-1]) {
                case WET: //Take wetwet probability
                    probability = wetwetProbability;
                    break;
                case DRY:// Take drywet probability
    ...
    public static int[] oneMonthForecast(int numberOfLocations, int month, double longitude, double latitude ){
        ...
        //need to create arrays to run populteArrays and populatelocationProbabilities 
        double[][] dwArr = new double[numberOfLocations][columns];
        double[][] wwArr = new double[numberOfLocations][columns];
        double[] dwProbability = new double[months];
        double[] wwProbability = new double[months];

        //call populateArrays method -> void method -> no returns
        populateArrays(dwArr, wwArr);
        
        //call populatelocationProbabilities method -> void method -> no returns
        populateLocationProbabilities(dwProbability, wwProbability, longitude, latitude, dwArr, wwArr);

        return forecastGenerator(dwProbability[month], wwProbability[month], numberOfDaysInMonth[month]);
    }
    ...
    public static int numberOfWetDryDays (int[] forecast, int mode) {
        ...
        int d = 0; //Keeps track for each day that is equal to mode

        //loops through each forecast array
        for (int i : forecast) {
            //day equals mode
            if (i == mode) {
                d++;
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
        ...
        //Create vars
        int longest = 0; //keeps track of longest spell
        int current = 0; //keeps track of current spell

        //loops through the array and if it find the current forcast is equal to mode then we increase current
        //else we reset current to 0 -> spell broke
        //then check if current is bigger than longest and sets new longest
    ...
    public static int bestWeekToTravel(int[] forecast){
        ...
        int bestWeek = 0; // tracks most dry days within a 7 day period
        int bestWeekIndex = 0; //tracks index to return
        int currentBestWeek = 0; //tracks current number of dry days

        //loops through the enitre array 
        for (int i = 0; i < forecast.length; i++) {
            currentBestWeek = 0;
            //loops to find the 7 days of best dry week
            for (int j = 0; j < 7; j++) {
                if(j+i >= forecast.length){ //edge case -> makes sure we dont go out of bounds
                    break;
                }
                if(forecast[j+i] == DRY){ // dry day is found
                    currentBestWeek++;
                }
            }
            if(currentBestWeek == 7){ //we found the first  perfect dry 7 days -> which is max so we can stop the loop
                bestWeekIndex=i;
                break;
            }
            if(currentBestWeek > bestWeek){
                bestWeek = currentBestWeek;
                bestWeekIndex = i;
            }
        }
